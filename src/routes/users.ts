...
router.put("/:id", async(req,res) => {
    if(req.body.name) {
        ...
        const user = await UserModel.findByIdAndUpdate(req.params.id, {name: req.body.name},{"new":true});
        return res.json(user);
    }
    if(req.body.username) {
        ...
        const user = await UserModel.findByIdAndUpdate(req.params.id, {username: req.body.username},{"new":true});
        return res.json(user);
    }
    if(req.body.surname) {
        const user = await UserModel.findByIdAndUpdate(req.params.id, {surname: req.body.surname},{"new":true});
        return res.json(user);
    }
